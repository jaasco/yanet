...
def create_packet(sni: str, ttl: int = 64, outgoing: bool = False):
    if outgoing:
...
write_pcap("send-002.pcap", [create_packet('good.example.com', ttl=64)])
write_pcap("recv-002.pcap", [create_packet('good.example.com', ttl=63, outgoing=True)])

write_pcap("send-003.pcap", [create_syn_packet()])
write_pcap("recv-003.pcap", [create_syn_packet()])

write_pcap("send-004.pcap", [create_packet('', ttl=64)])
write_pcap("recv-004.pcap", [create_packet('', ttl=63, outgoing=True)])

handshake_packets = [syn(), synack(), ack(), make_half_tls_handshake_packet()]
wrpcap("send-005.pcap", handshake_packets)