...
send_udp = Ether(src=client_mac, dst=lp_mac) / Dot1Q(vlan=vlan) / \
           IP(src=client_ip, dst=server_ip, ttl=64) / \
           UDP(sport=sport, dport=dport) / b"hello"

recv_udp = Ether(src=client_mac, dst=lp_mac) / Dot1Q(vlan=vlan) / \
           IP(src=client_ip, dst=server_ip, ttl=64) / \
           UDP(sport=sport, dport=dport) / b"hello"
...
send_icmp = Ether(src=client_mac, dst=lp_mac) / Dot1Q(vlan=vlan) / \
            IP(src=client_ip, dst=server_ip, ttl=64) / \
            ICMP(type="echo-request") / b"ping"

recv_icmp = Ether(src=client_mac, dst=lp_mac) / Dot1Q(vlan=vlan) / \
            IP(src=client_ip, dst=server_ip, ttl=64) / \
            ICMP(type="echo-request") / b"ping"
