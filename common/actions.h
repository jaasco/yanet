...
struct CheckSniAction final
{
	...
	CheckSniAction(const acl::check_sni_t& sni_action) :
	        sni(sni_action.sni),
	        flow(common::globalBase::tFlow(common::globalBase::eFlowType::drop)) {};

	CheckSniAction(const std::string& sni_, const common::globalBase::tFlow& flow_) :
	        sni(sni_), flow(flow_) {}

	CheckSniAction() :
	        sni(""), flow(globalBase::tFlow())
	{
		{};

		[[nodiscard]] bool terminating() const { return false; }

		bool operator==(const CheckSniAction& other) const
		{
			return sni == other.sni && flow == other.flow;
		}

		SERIALIZABLE(sni, flow);

		[[nodiscard]] std::string to_string() const
		{
			std::ostringstream oss;
			oss << "CheckSniAction(sni=" << sni << ", flow=" << flow.to_string() << ")";
			return oss.str();
		}
	};
